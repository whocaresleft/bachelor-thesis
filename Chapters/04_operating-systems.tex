...
\begin{itemize}
    ...
    \item Implementazione di un \texttt{trait} non sicuro;
...
di conseguenza in tali contesti viene restituito \texttt{Ok(())} (ovvero, `\textit{successo senza un valore significativo}'). \hfill

Un altro aspetto fondamentale del linguaggio è quello dei \texttt{panic}, ovvero un ulteriore meccanismo impiegato nella gestione degli errori.
Dal punto di vista esterno, sono molto simili alle eccezioni di Java, ovvero causano l'interruzione dell'esecuzione.
Tuttavia, internamente al processo, i \texttt{panic} procedono in maniera leggermente diversa rispetto alle eccezioni, causando uno \textit{stack unwinding}\footnote{Conviene sottolineare che, anche in Java, si verifica uno \texttt{stack unwinding}, in cui viene percorso lo stack delle chiamate e, contemporaneamente, vengono marcati tutti gli oggetti come non referenziati. Anche questo provoca la deallocazione della memoria; tuttavia, non vi è garanzia riguardo le altre possibili risorse allocate (per esempio, un file aperto potrebbe rimanere tale). In Rust, la maggior parte delle strutture che allocano risorse, definiscono \texttt{drop} in maniera tale da liberarle; di conseguenza, anche in presenza di un \texttt{panic}, verrebbero deallocate correttamente.}:
quando viene generato un \texttt{panic}, Rust percorre lo stack delle chiamate delle funzioni, invocando \texttt{drop} su ogni \textit{owner} presente. Questo permette 
una gestione sicura perfino degli errori improvvisi, liberando tutte le risorse allocate al momento dell'errore\footnote{Questo, tuttavia, non è sempre vero in pratica. Oltre allo \textit{stack unwinding}, Rust permette di selezionare \textit{abort} come \textit{gestore di \texttt{panic}}. In questa configurazione, il processo viene terminato senza liberare le risorse, con un comportamento analogo a un \texttt{exit(FAILURE)} improvviso in C. La modifica del gestore di \texttt{panic} deve avvenire in maniera esplicita, in quanto Rust utilizza, di default, \textit{stack unwinding}.}.
\vspace{10pt}\\
...
gestiti esplicitamente\footnote{In realtà, per \texttt{Result} è disponibile una funzione, \texttt{unwrap}, la quale tenta di restituire il contenuto di \texttt{Ok}, generando un \texttt{panic} se al suo posto era presente un \texttt{Err}. Questa funzione andrebbe utilizzata quando si è certi che il \texttt{Result} su cui la si invoca contenga un \texttt{Ok}. Va considerato che, il comportamento ideomatico di Rust, prevederebbe di controllare il contenuto, verificando che sia presente un \texttt{Ok} o un \texttt{Err}, prima di accedere al valore di \texttt{Result}. Alternativamente, \texttt{unwrap} potrebbe essere utilizzato come indicatore che un eventuale errore in quel punto sia irrecuperabile (seguendo il pensiero ideomatico di Rust, in cui \texttt{panic} è da usare solo per gli errori irrecuperabili). } oppure che l'errore venga propagato al chiamante tramite \texttt{?}, prevenendo scenari di \textit{undefined behaviour}.
...
in Figura~\ref{c:access-after-free-exec}. \hfill
\vspace{12pt}\\
\noindent Nel Listato~\ref{rust:access-after-free} è riportato un programma che mostra come Rust gestisce questa problematica.
...
\end{lstlisting}
Il Listato~\ref{rust:access-after-free} tenta di replicare il comportamento del Listato~\ref{c:access-after-free}, cercando 
...
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=1.0\textwidth]{access-after-free-c-exec}
...
In questo caso il programma termina senza errori, viene anche eseguita la stampa finale, come mostrato in Figura~\ref{c:buffer-overflow-exec}. 
...
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=1\textwidth]{c-buffer-overflow-exec}
    \caption{\textit{Buffer overflow} in C}\label{c:buffer-overflow-exec}
    \end{center}
\end{figure}
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=1\textwidth]{rust-buffer-overflow-exec}
...
In questo caso il programma termina senza errori, stampando effettivamente dieci elementi, come mostrato in Figura~\ref{c:buffer-overread-exec}. 
...
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=1\textwidth]{c-buffer-overread-exec}
    \caption{\textit{Buffer overread} in C}\label{c:buffer-overread-exec}
    \end{center}
\end{figure}
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=1\textwidth]{rust-buffer-overread-exec}
...
Di conseguenza il valore letto, questa volta, è diverso da zero, come si può osservare in Figura~\ref{c:uninitialized-memory-access-exec-2}\footnote{Anche in questo caso, si tratta di un comportamento specifico del sistema e non garantito dallo standard.}.
\begin{figure}[htbp]
\begin{center}
    \includegraphics[width=0.8\textwidth]{c-uninitialized-memory-access-exec}
...
\subsubsection{Conclusioni}
Riassumendo, sotto l'aspetto della sicurezza della memoria, Rust si distingue da C garantendo un comportamento sicuro e deterministico, prevenendo
...
Infatti, nonostante quest'ultime siano definite \textit{zero-cost} in teoria, nella pratica, spesso, il compilatore non riesce a ottimizzarle pienamente e possono introdurre un minimo overhead\footnote{Per esempio, si consideri \texttt{Option<T>}, enumerazione che permette di esprimere la possibile assenza di un valore, \texttt{None} e \texttt{Some<T>}. In quanto Rust non permette che un riferimento sia nullo, tipi come \texttt{Box} o \texttt{String} non potranno mai valere 0. In questo scenario, \texttt{Option<Box>} e \texttt{Option<String>} occuperebbero lo stesso spazio di \texttt{Box} e \texttt{String} rispettivamente: il compilatore ottimizza l'astrazione, utilizzando il valore \texttt{0} per \texttt{None} e considerando ogni valore diverso da \texttt{0} come \texttt{Some}. Tuttavia, per tipi che possono essere nulli, come \texttt{u8}, \texttt{Option<u8>} richiede un byte aggiuntivo per rappresentare l'assenza di valore, non permettendo al compilatore di ottimizzare l'uso dell'astrazione.}.
...
Nei Listati~\ref{rust:lifetime-prevention} e~\ref{c:lifetime-prevention} sono riportati due esempi che mostrano come le annotazioni di \textit{lifetime} possano prevenire \textit{access after free}.
...
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=1.0\textwidth]{c-aaf-lifetime}
        \caption{\textit{Access after free} dovuto all'assenza di \textit{lifetime} in C}\label{c:aaf-lifetime}
    \end{center}
\end{figure}
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=1.0\textwidth]{rust-aaf-lifetime}
...
Rust consente inoltre di specificare vincoli sui tipi generici (\textit{\texttt{trait} bounds}), tramite \texttt{impl Trait} e \texttt{where}. Questi meccanismi sono equivalenti, rispettivamente, a \texttt{implements Interface} e \texttt{<T extends \ldots>} di Java. \hfill
\vspace{8pt}\\
...
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{c-macro-preprocess}
...
come è osservabile in Figura~\ref{c:macro-exec}. \hfill
\begin{figure}[htbp]
    \begin{center}
...
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=1\textwidth]{rust-correct-macro}
        \caption{Compilazione di una \textit{macro} in Rust}\label{rust:correct-macro}
    \end{center}
\end{figure}
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=1\textwidth]{rust-incorrect-macro}
