...
verrà analizzata la gestione di: memoria dinamica, file su disco, risorse di rete e thread con risorse condivise\footnote{Gli esempi di codice riportati in questa sottosezione sono stati sviluppati su ambiente Linux con \texttt{glibc}}.
...
int main(void) {
    ...
    int* value = malloc(sizeof(int)); // Allocation
    if(value == NULL) return -1;

    *value = 52; // Initialization

    printf("%d\n", *value); // Access

    free(value);    // Deallocation
...
fn main() {
    ...
    let value = Box::<i32>::new(52); // RAII: Resource Allocation Is Initialization

    println!("{}", *value); // Access

} // <-- End of scope: automatic deallocation
\end{lstlisting}
...
fn main() -> std::io::Resilt<()> {

    let server = TcpListener::bind("0.0.0.0:50000")?;
